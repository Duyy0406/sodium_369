...
interface LinkPropsNext
  extends Partial<Pick<NextLinkProps, "shallow" | "prefetch" | "scroll">> {}

export interface LinkProps extends LinkPropsChakra, LinkPropsNext {}

const splitProps = (
  props: LinkProps
): { chakra: LinkPropsChakra; next: NextLinkProps } => {
  ...
  function Link(props, ref) {
    ...
    return (
      ...
      >
        ...
        >
          {next.href ? (
            <NextLink {...next}>{children}</NextLink>
          ) : (
            <span>{children}</span>
          )}
        </ChakraLink>
